...
export const getCalendar = defineTool({
  ...
  async handler({ startDay, endDay }) {
    ...
    const lines = entities.map((entity) => {
      const program = programByIdInPlan.get(
        String(entity.planProgramId),
      );
      const name =
        entity.sportData?.name ?? program?.name ?? 'Unknown';
      const sport =
        entity.sportData?.sportType ?? program?.sportType;
      const sportLabel =
        sport != null
          ? (SPORT_TYPE_LABELS[sport] ?? `type ${sport}`)
          : 'Unknown';
      const load =
        entity.sportData?.trainingLoad ??
        program?.trainingLoad ??
        0;
